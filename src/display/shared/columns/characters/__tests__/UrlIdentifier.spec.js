...
describe('data', () => {
	...
	it('should display character identifier', () => {
		const cellJsx = column.Cell({ original: { isOnHiatus: true }, value: 'my-url-identifier' });
		const cellElement = shallow(cellJsx);
		expect(cellElement.find('a')).toHaveText('my-url-identifier');
	});
	it('should link to character home URL', () => {
		const cellJsx = column.Cell({ original: { isOnHiatus: true, homeUrl: 'http://my-url-identifier.tumblr.com' } });
		const cellElement = shallow(cellJsx);
