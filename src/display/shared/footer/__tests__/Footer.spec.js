...
import { initMockDateNow, getSpecWrapper } from '../../../../../config/tests/helpers.unit';
...
describe('rendering', () => {
	describe('snapshots', () => {
		...
		it('should render valid snapshot when light theme', () => {
			const jsx = (<Footer {...createTestProps({ useLightTheme: true })} />);
			const element = shallow(jsx);
			expect(element).toMatchSnapshot();
		});
	});
...
describe('behavior', () => {
	describe('theme toggle', () => {
		it('should trigger theme toggle on button click', () => {
			...
			const props = createTestProps({ toggleTheme });
