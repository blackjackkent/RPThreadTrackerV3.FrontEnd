...
jest.mock('../../../../forms/update-account-info/UpdateAccountInfoForm', () => 'UpdateAccountInfoForm');
...
describe('behavior', () => {
	describe('componentWillReceiveProps', () => {
		it('should set character to edit', () => {
			...
			const user = { id: '12345', email: 'test@test.com', username: 'my-test-username' };
			const props = createTestProps({ submitAccountInfoForm, user: {} });
	...
	describe('handleInputChange', () => {
		it('should handle text field update', () => {
			const event = { target: { type: 'text', name: 'username', value: 'my-new-username' } };
			const submitAccountInfoForm = jest.fn();
			const props = createTestProps({ submitAccountInfoForm });
			const element = shallow(<UpdateAccountInfoPane {...props} />);
			...
			form.props().onValidSubmit();
			expect(submitAccountInfoForm).toHaveBeenCalledTimes(1);
			expect(submitAccountInfoForm).toHaveBeenLastCalledWith({ username: 'my-new-username' });
		});
	});
});
