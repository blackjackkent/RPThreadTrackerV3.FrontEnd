...
const useAllTags = () => {
	const { filteredThreads: allActiveThreads, isThreadsLoading: isActiveThreadsLoading } =
		useFilteredActiveThreads(filters.ALL);
	const { filteredThreads: allArchivedThreads, isThreadsLoading: isArchivedThreadsLoading } =
		useArchivedThreads();
	const allThreads = useMemo(
		() => allActiveThreads.concat(allArchivedThreads),
		[allActiveThreads, allArchivedThreads]
	);
